from evaluate.functions import *
...
def microF1(y_test, predictions):
    precmicro=precisionMicro(y_test,predictions)
    recmicro=recallMicro(y_test,predictions)
    microF1=2*(precmicro*recmicro)/(precmicro+recmicro)
    return microF1

def macroF1(y_test, predictions):
    precmacro = precisionMacro(y_test, predictions)
    recmacro = recallMacro(y_test, predictions)
    macroF1 = 2 * (precmacro * recmacro) / (precmacro + recmacro)
    return macroF1
