...
class ClassificationEvaluator(object):
    ...
    def _calculate_prf(right_count, predict_count, standard_count):
        ...
        if predict_count > 0:
            precision = right_count / predict_count #precision = TP / (TP + FP)
        if standard_count > 0:
            recall = right_count / standard_count   #recall = TP / (TP + FN)
    ...
    def evaluate(self, predicts, standard_label_names=None,
        ...
        def calculate_ROC_epoch(actual, pred_b):
            ...
            pr_auc["micro"] = sk.auc(recall["micro"], precision["micro"])

            return roc_auc['micro'], pr_auc["micro"]
